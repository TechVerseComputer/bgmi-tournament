...
import { Gamepad, Crosshair, Users, Trophy, X, AlertCircle, Wallet } from 'lucide-react';
...
export default function TournamentsPage() {
  ...
  const handleJoinClick = async (match: any) => {
    ...
    // Fetch which slots are already taken for this specific match
    const { data: regs } = await supabase.from('registrations').select('slot_number').eq('tournament_id', match.id);
    if (regs) {
      setBookedSlots(regs.map(r => r.slot_number).filter(s => s !== null));
    }
  };

  // Process Registration & Deduct Wallet
  const handleConfirmBooking = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error: regError } = await supabase.from('registrations').insert([{
        ...
        user_id: user.id,
        squad_name: team.p1_ign + "'s Squad", // Using Captain's IGN as squad name for now
        igl_email: user.email,
        player_1_id: team.p1_id, player_1_ign: team.p1_ign,
        player_2_id: team.p2_id, player_2_ign: team.p2_ign,
        player_3_id: team.p3_id, player_3_ign: team.p3_ign,
        player_4_id: team.p4_id, player_4_ign: team.p4_ign,
  ...
  return (
    <main className="min-h-screen bg-[#0a0a0a] text-white font-sans selection:bg-orange-500 selection:text-white relative">
      
      {/* Navigation */}
      <nav className="w-full z-40 p-4 lg:px-12 flex justify-between items-center bg-zinc-950 border-b border-zinc-900 sticky top-0">
        ...
        <div className="hidden md:flex gap-8 text-sm font-bold tracking-wide items-center">
          ...
          <Link href="/dashboard" className="bg-zinc-800 hover:bg-zinc-700 px-4 py-2 rounded flex items-center gap-2 transition-colors border border-zinc-700">
            <Wallet className="w-4 h-4 text-emerald-500" /> {user ? `₹${walletBalance}` : 'Wallet'}
          </Link>
        </div>
      </nav>
            ...
            {walletBalance < selectedMatch.fee && (
              <div className="bg-red-500/10 border-b border-red-500/20 p-4 flex items-center justify-between">
                <div className="flex items-center gap-2 text-red-500 text-sm font-bold">
                  <AlertCircle className="w-5 h-5" /> Insufficient Wallet Balance (₹{walletBalance})
                </div>
            ...
            <form onSubmit={handleConfirmBooking} className="p-6 space-y-6">
                  ...
                  {[1, 2, 3, 4].map((num) => (
                    <div key={num} className="bg-zinc-900/50 p-4 rounded-lg border border-zinc-800/50">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <input required type="text" placeholder="In-Game Name (IGN)" 
                          value={team[`p${num}_ign` as keyof typeof team]} 
                          onChange={e => setTeam({...team, [`p${num}_ign`]: e.target.value})} 
                          className="w-full bg-zinc-950 border border-zinc-800 rounded p-3 text-sm focus:border-orange-500 outline-none text-zinc-300" />
                        <input required type="text" placeholder="Game ID (Numbers)" 
                          value={team[`p${num}_id` as keyof typeof team]} 
                          onChange={e => setTeam({...team, [`p${num}_id`]: e.target.value})} 
                          className="w-full bg-zinc-950 border border-zinc-800 rounded p-3 text-sm focus:border-orange-500 outline-none font-mono text-zinc-300" />
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-5 gap-2">
                  {Array.from({ length: 25 }, (_, i) => i + 1).map((slot) => {
                    const isBooked = bookedSlots.includes(slot);
                    const isSelected = selectedSlot === slot;
                    return (
                      <button type="button" key={slot} disabled={isBooked} onClick={() => setSelectedSlot(slot)}
                        className={`py-3 rounded text-sm font-black transition-all ${
                          isBooked ? 'bg-red-500/10 text-red-500/50 border border-red-500/10 cursor-not-allowed' :
                          isSelected ? 'bg-orange-500 text-black border-2 border-orange-500 shadow-[0_0_15px_rgba(249,115,22,0.4)]' :
